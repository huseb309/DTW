...
const moment = require('moment-timezone'); // Added for WIB timezone handling
...
function tulisLog(teks, nomor = null, status = null, scheduleId = null) {
  const timestamp = moment().tz('Asia/Jakarta').format('YYYY-MM-DD HH:mm:ss'); // Use WIB timezone
...
app.get('/download_logs', authMiddleware, (req, res) => {
  try {
    ...
    logsDb.all(query, params, (err, rows) => {
      ...
      rows.forEach(row => {
        // Convert UTC timestamp to WIB for existing logs
        const wibTimestamp = moment(row.timestamp).tz('Asia/Jakarta').format('YYYY-MM-DD HH:mm:ss');
        content += `${wibTimestamp} | ${row.teks}\n`;
      });
